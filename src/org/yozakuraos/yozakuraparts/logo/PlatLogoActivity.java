...
package org.yozakuraos.yozakuraparts.logo;
...
public class PlatLogoActivity extends Activity {
    ...
    private class PBackground extends Drawable {
        ...
        // YozakuraOS sakura logo drawable
        private final Drawable mLogo;

        public PBackground(Context context) {
            randomizePalette();
            // YozakuraOS sakura logo
            mLogo = context.getResources().getDrawable(R.drawable.logo_yozakura, context.getTheme());
        ...
        public void randomizePalette() {
            // YozakuraOS: sakura-night palette (pinks fading to white), not a random rainbow
            final int slots = 3 + (int)(Math.random() * 2);
            mPalette = new int[slots];
            mDarkest = 0;
            for (int i = 0; i < slots; i++) {
                // hue clustered around sakura pink; deeper/more saturated toward the inner circles
                final float hue = 325f + (float) Math.random() * 25f;
                final float sat = 0.14f + ((float) i / (float) slots) * 0.5f;
                mPalette[i] = Color.HSVToColor(new float[] { hue, sat, 1f });
        ...
        public void draw(Canvas canvas) {
            ...
            /* YozakuraOS: solid disc backdrop for the sakura logo
             * (replaces the old LineageOS logo built out of circles).
             */
            paint.setColor(mPalette[mDarkest]);
            paint.setStyle(Paint.Style.FILL);
            canvas.drawCircle(0, 0, mRadius * 1.5f, paint);
    ...
    private void checkLongPressTimeout() {
        if (mTouchHeld > 0 && mTapCount >= 5) {
            if (System.currentTimeMillis() - mTouchHeld >= LONG_PRESS_TIMEOUT) {
                ...
                mLayout.post(() -> {
                    try {
                        startActivity(new Intent("org.yozakuraos.yozakuraparts.EASTER_EGG")
